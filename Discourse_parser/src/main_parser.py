...
# from eval.evaluation_local import Evaluator
...
def train_model(data_helper):
    ...
    rst_parser.save(model_dir='Discourse_parser/data/model')
...
if __name__ == '__main__':
    args = parse_args()
    # Use brown clusters
    with gzip.open("Discourse_parser/data/resources/bc3200.pickle.gz") as fin:
    ...
    if args.prepare:
        ...
        data_helper.save_data_helper('Discourse_parser/data/data_helper.bin')
    if args.train:
        data_helper.load_data_helper('Discourse_parser/data/data_helper.bin')
        data_helper.load_train_data(data_dir=args.train_dir)
        train_model(data_helper)
    if args.eval:
        # Evaluate models on the RST-DT test set
        evaluator = Evaluator(model_dir='Discourse_parser/data/model')
        print("calling eval parser .. ")
        evaluator.eval_parser(path=args.eval_dir, report=False, bcvocab=brown_clusters, draw=False, 
