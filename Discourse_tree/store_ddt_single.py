        ...
        for (s, _) in dfs_res:
            if s not in already_used_sources:
                ...
                if len(updated_nodes) == 4 and len(updated_nodes_set) == 3:
                    if updated_nodes[1] == updated_nodes[2] and children[0] in sentence_tree_to_edu_tree_mapping and all(x in sentence_tree_to_edu_tree_mapping for x in [children[0], children[1]]):
            ...
            if len(leaky_node) == 1 and leaky_node[0] == culprit_in_leaky_node:
                ...
                if updated_node_first_part != updated_node_second_part:
                    ...
                    if updated_node in discourse_tree_sentence.nodes:
                        ...
                        if list(discourse_tree_sentence.successors(leaky_node_right)):
