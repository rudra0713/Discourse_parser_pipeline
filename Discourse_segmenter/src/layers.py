...
import tensorflow.compat.v1.nn.rnn_cell as rnn_cell
...
def trilinear_similarity(x1, x2, scope='trilinear', reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        weights_x1 = tf.compat.v1.get_variable('kernel_x1', [x1_shape[2], 1], dtype=x1.dtype)
        weights_x2 = tf.compat.v1.get_variable('kernel_x2', [x2_shape[2], 1], dtype=x2.dtype)
        weights_mul = tf.compat.v1.get_variable('kernel_mul', [1, 1, x1_shape[2]], dtype=x2.dtype)
        bias = tf.compat.v1.get_variable('bias', [1], dtype=x1.dtype, initializer=tf.zeros_initializer)
...
def self_attention(inputs, lengths, window_size=-1, scope='bilinear_attention', reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        if window_size > 0:
            ...
            restricted_mask = tf.compat.v1.matrix_band_part(tf.ones_like(logits, dtype=tf.float32), window_size, window_size)
...
def get_cell(rnn_type, hidden_size, layer_num=1, dropout_keep_prob=None):
    ...
    for i in range(layer_num):
        if rnn_type.endswith('lstm'):
            ...
            # could not find a way to replace this contrib call
            ...
            # could not find a way to replace this contrib call
            cell = rnn_cell.GRUCell(num_units=hidden_size)
    ...
    cells = rnn_cell.MultiRNNCell(cells, state_is_tuple=True)
