...
def segment_mp(args):
    ...
    articles_only = pickle.load(open(args.input_files[0], "rb"))
    articles_list = [articles_only[key] for key in articles_only]
    processes = [Process(target=segment, args=(args, start_index, rst_data, model, articles_list)) for start_index in range(0, 10, 2)]
    ...
    pickle.dump(results, open(args.result_dir + os.sep + args.result_file + ".p", "wb"))
    return


def segment(args, start_index, rst_data, model, articles_list):
    """
    ...
    edu_output = {}
    print("args input files ", args.input_files)
    articles_only = pickle.load(open(args.input_files[0], "rb"))
    count_done = 0
    print("starting now")
    for ind_article in range(start_index, start_index + 2):
        print("ind article -> ", ind_article)
        raw_sents = [articles_list[ind_article]]
        samples = []
        for sent in spacy_nlp.pipe(raw_sents, batch_size=1000, n_threads=5):
            # print("sent ", sent)
            # print("tokens ", [token.text for token in sent])
        ...
        for batch in data_batches:
            try:
                ...
                #     fout.write(' '.join(sent for sent in raw_sents) + '\n')

                # print(articles_only[key])
                # break
                # sys.exit(0)
            for sample, pred_segs in zip(batch['raw_data'], batch_pred_segs):
                ...
                for word_idx, word in enumerate(sample['words']):
                    # print("word ", word_idx, word)
                    if word_idx in pred_segs:
                        if len(one_edu_words) > 1:   # condition added by rudra
        ...
        if len(edus) == 0:
            edus = [' '.join(sent for sent in raw_sents)]

        # save_path = os.path.join(args.result_dir, os.path.basename(key)) + '.edus' # modified by rudra
        edu_output['test_' + str(ind_article)] = edus
        print(count_done)
        # if count_done == 5:
        #     break
        ...
        #         fout.write(edu + '\n')
    count_done += 1
    print("count done ", count_done, "writing in ", args.result_dir)
    queue.put(edu_output)


    # pickle.dump(edu_output, open(args.result_dir + os.sep + "rfd_claim_with_articles_edus.p", "wb"))
    # pickle.dump(edu_output, open("../output/fnc_claim_with_articles_edus.p", "wb"))