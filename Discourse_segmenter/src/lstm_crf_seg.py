...
class LSTMCRFSegModel(LSTMSegModel):
    ...
    def _output(self):
        ...
        # self.scores = tf.compat.v1.layers.dense(self.encoded_sent, 2, activation=None, scope='output_fc1')
        self.scores = tf.compat.v1.layers.dense(self.encoded_sent, 2, activation=None)
        ...
        self.log_likelyhood, self.trans_params = tfa.text.crf_log_likelihood(self.scores,
                                                                           tf.cast(self.placeholders['seg_labels'], tf.int32),
                                                                           self.placeholders['input_length'])
