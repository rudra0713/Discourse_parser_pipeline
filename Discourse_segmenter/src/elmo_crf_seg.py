...
class ELMOCRFSegModel(LSTMCRFSegModel):
    ...
    def __init__(self, args, word_vocab):
        print("inside __init__ method of elmo_crf_seg.py")
        super().__init__(args, word_vocab)

        # import ElmoEmbedder here so that the cuda_visible_divices can work
        from allennlp.commands.elmo import ElmoEmbedder
        # self.elmo = ElmoEmbedder(cuda_device=0 if args.gpu is not None else -1) # changed by me to the following line
        self.elmo = ElmoEmbedder(cuda_device=0 if args.gpu else -1)
    ...
    def _setup_placeholders(self):
        ...
        self.placeholders = {'input_words': tf.compat.v1.placeholder(tf.int32, shape=[None, None]),
                             'input_length': tf.compat.v1.placeholder(tf.int32, shape=[None]),
                             'elmo_vectors': tf.compat.v1.placeholder(tf.float32, shape=[None, 3, None, 1024]),
    ...
    def _embed(self):
        with tf.device('/cpu:0'):
            ...
            #                                        trainable=False)
            self.word_embeddings = tf.compat.v1.get_variable('word_embeddings',
                                                   shape=(self.word_vocab.size(), self.word_vocab.embed_dim),
                                                   initializer=word_emb_init,
                                                   trainable=False)
        ...
        self.elmo_weights = tf.nn.softmax(tf.compat.v1.get_variable('elmo_weights', [3], dtype=tf.float32, trainable=True))
        # self.scale_para = tf.get_variable('scale_para', [1], dtype=tf.float32, trainable=True)
        self.scale_para = tf.compat.v1.get_variable('scale_para', [1], dtype=tf.float32, trainable=True)
    ...
    def _compute_loss(self):
        ...
        if self.weight_decay > 0:
            ...
            with tf.compat.v1.variable_scope('l2_loss'):
                ...
                l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in tf.compat.v1.trainable_variables() if 'bias' not in v.name])
    ...
    def segment(self, batch):
        ...
        for sample_idx in range(len(batch['raw_data'])):
            ...
            viterbi_seq, viterbi_score = tfa.text.viterbi_decode(scores[sample_idx][:length], trans_params)
