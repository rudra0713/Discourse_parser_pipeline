...
def get_cell(rnn_type, hidden_size, layer_num=1, dropout_keep_prob=None):
    if rnn_type.endswith('lstm'):
        ...
        # cell = tf.keras.layers.LSTMCell(num_units=hidden_size, state_is_tuple=True)
    ...
    if dropout_keep_prob is not None:
        ...
        cell = rnn_cell.DropoutWrapper(cell, input_keep_prob=dropout_keep_prob, output_keep_prob=dropout_keep_prob)

    if layer_num > 1:
        # cell = tc.rnn.MultiRNNCell([cell]*layer_num, state_is_tuple=True)
        cell = rnn_cell.MultiRNNCell([cell]*layer_num, state_is_tuple=True)
